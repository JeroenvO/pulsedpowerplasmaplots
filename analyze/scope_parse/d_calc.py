...
def calc(line):
    ...
    before, pulse = find_longest_ranges(ranges, 2)
    i_time_settling = pulse[0]-pulse[1]
    v_pulse = np.mean(v[i_time_settling:pulse[0]])  # average of voltage during pulse when current is < 5% of max current
    ...
    #v_time_settling = pulse[0]-pulse[1]
    t_settling_end = t[pulse[0]-pulse[1]]
    v05 = 0.05*v_pulse
    t_settling_start = t[np.where(v>v05)[0][0]] # when v first rises above 0.05 of final
