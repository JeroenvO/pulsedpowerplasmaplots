...
def correct_lines(line_objs, time_point = 1e-9, div_zero = 128.0, div_point = 25, offsets=[]):
    ...
    :param offsets: array for each data line with [v_shift,
    ...
    for i, line in enumerate(line_objs[1:]): # first line_obj is generic data.
        ...
        if offsets:
            if offsets[i-1]['val_div_correct']: # sometimes val_div is not correctly exported. 'mili' tends to be ignored.
                y /= offsets[i-1]['val_div_correct']
            if offsets[i-1]['v_shift']:  # vertical shift of n-elements to account for delay in probe/cables.
                np.roll(y, offsets[i-1]['v_shift'])
        y_axes.append(y)
