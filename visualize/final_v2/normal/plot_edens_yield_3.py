...
def plot_edens_yield(data):
    ...
    for reactor, ind in [(REACTOR_GLASS_LONG, 26), (REACTOR_GLASS_SHORT_QUAD, 0), (REACTOR_GLASS_SHORT_QUAD, 26)]:
        ...
        ax[3].scatter(edens, y, label=l, c=c, marker=m)
        fit = np.polyfit(edens, y, 1)
        fit_fn = np.poly1d(fit)
        ax[3].plot(edens, fit_fn(edens), '--', c=c, lw=0.9)

        y= get_values(data, 'output_e_plasma')
        ax[4].scatter(edens, y, c=c, marker=m)
        fit = np.polyfit(edens, y, 1)
        fit_fn = np.poly1d(fit)
    ...
    ax[4].set_ylim([0.001, 0.015])
    ax[4].set_xlabel('Energy density [J/l]')

    set_unique_legend(ax[1])
    set_plot(fig, plot_height=4)
    save_file(fig, name='edens-all_3', path='plots_final_v2/normal')
